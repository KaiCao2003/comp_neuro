...
function StepLead({ number, title, children, id }: { number: number; title: string; children?: ReactNode; id?: string }) {
  return <div className={styles.sectionLead}><p className={styles.stepNumber}>{number}</p><div><h4 id={id}><ScientificText text={title} /></h4>{children}</div></div>;
}
...
export function StudyModule({ module, locale = 'zh', figures = [], afterFigure }: { module: StudyModuleData; locale?: Locale; figures?: FigureIndexEntry[]; afterFigure?: ReactNode }) {
  ...
  return <article className={`study-module ${styles.module}`} id={module.id}>
    <header>
      ...
      <h3><ScientificText text={module.title} /></h3>
    ...
    <section className={styles.entry} aria-labelledby={`${module.id}-entry`}>
      ...
      <p className="guiding-question"><strong>{t('核心问题：', 'Core question: ')}</strong><ScientificText text={module.guidingQuestion} /></p>
      {!progress.opened && <>
        ...
        {hintLevel > 0 && <ol className={styles.hints} aria-live="polite">{hints.slice(0, hintLevel).map((hint, index) => <li key={`${module.id}-hint-${index}`}><ScientificText text={hint} /></li>)}</ol>}
    ...
    <div className={progress.opened ? styles.revealed : styles.concealed}>
      <section className={styles.teaching} aria-labelledby={`${module.id}-teaching`}>
        ...
        <div className="study-key-points"><h4>{t('读完应能独立解释', 'Explain independently after reading')}</h4><ul>{module.keyPoints.map((point) => <li key={point}><ScientificText text={point} /></li>)}</ul></div>
      </section>

      {module.derivation && <section className="study-derivation" aria-labelledby={`${module.id}-derivation`}>
        <StepLead number={3} id={`${module.id}-derivation`} title={module.derivation.title}><p><ScientificText text={module.derivation.setup} /></p><p className={styles.instruction}>{t('只先看每步标题，预测下一步后再展开。', 'Read each step title, predict the next move, then expand it.')}</p></StepLead>
        <ol className={`derivation-steps ${styles.derivationSteps}`}>{module.derivation.steps.map((step, index) => <li key={`${module.id}-step-${index}`}><details><summary><strong><ScientificText text={step.title} /></strong></summary><p><ScientificText text={step.explanation} /></p>{step.latex && <MathStep latex={step.latex} label={`${module.derivation?.title}: ${step.title}`} />}</details></li>)}</ol>
      ...
      <section className={`worked-example ${styles.worked}`} aria-labelledby={`${module.id}-example`}>
        ...
        <div className={progress.example ? styles.solutionRevealed : styles.solutionConcealed}><h5>{t('逐步解答', 'Step-by-step solution')}</h5><ol>{module.workedExample.steps.map((step) => <li key={step}><ScientificText text={step} /></li>)}</ol><p><strong>{t('答案。', 'Result. ')}</strong><ScientificText text={module.workedExample.result} /></p><p><strong>{t('检查。', 'Check. ')}</strong><ScientificText text={module.workedExample.sanityCheck} /></p></div>
      </section>
      ...
      <aside className={`study-self-check ${styles.selfCheck}`} aria-labelledby={`${module.id}-self-check`}>
        ...
        <p className={styles.selfPrompt}><ScientificText text={module.selfCheck.prompt} /></p>
        ...
        <div className={progress.selfCheck ? styles.solutionRevealed : styles.solutionConcealed}><h5>{t('参考答案', 'Reference answer')}</h5><p><ScientificText text={module.selfCheck.answer} /></p><p className={styles.comparePrompt}>{t('逐句比较：遗漏的是概念、假设、推导步骤，还是迁移能力？', 'Compare sentence by sentence: did you miss a concept, assumption, derivation step, or transfer ability?')}</p></div>
      </aside>

      <div className="study-pitfalls"><h4>{t('用反例检查常见错误', 'Test common failure modes')}</h4><ul>{module.pitfalls.map((pitfall) => <li key={pitfall}><ScientificText text={pitfall} /></li>)}</ul></div>
