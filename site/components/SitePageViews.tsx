...
export function CourseMapView({ locale, course, dependencies }: { locale: Locale; course: CourseSummary[]; dependencies: Edge[] }) {
  return <main className="index-page wide-page" id="main-content"><header><p className="eyebrow">Dependency Map</p><h1>{locale === 'zh' ? '课程图谱' : 'Course map'}</h1></header><CourseDependencyGraph locale={locale} course={course} dependencies={dependencies} /><ol className="dependency-list">{course.map((lecture) => {
    ...
    return <li key={lecture.lecture}><Link className="dependency-title" href={localizedHref(locale, `/lectures/${lecture.slug}/`)}>{lecture.lecture} · <ScientificText text={locale === 'zh' ? lecture.zhTitle : lecture.enTitle} /></Link><span>{prereqs.length ? <>{locale === 'zh' ? '前置：' : 'Prerequisites: '}{prereqs.map((item, index) => <span key={item!.lecture}>{index > 0 ? (locale === 'zh' ? '、' : ', ') : ''}<Link href={localizedHref(locale, `/lectures/${item!.slug}/`)}>{item!.lecture}</Link></span>)}</> : (locale === 'zh' ? '起点' : 'Starting point')}</span></li>;
  })}</ol></main>;
}

export function ErrataView({ locale, errata }: { locale: Locale; errata: Erratum[] }) {
  return <main className="index-page" id="main-content"><header><p className="eyebrow">Errata &amp; Uncertainty</p><h1>{locale === 'zh' ? '勘误与不确定项' : 'Errata and uncertainties'}</h1><p className="index-count">{errata.length}</p></header><ol className="reference-list errata-list">{errata.map((item) => <li key={item.id}><p className="reference-kicker">{locale === 'zh' ? `第 ${item.lecture} 讲` : `Lecture ${item.lecture}`} · <ScientificText text={item.lectureTitle} /> · {item.sourceFile}{item.sourcePage ? (locale === 'zh' ? ` · 第 ${item.sourcePage} 页` : ` · p. ${item.sourcePage}`) : ''}</p><p><strong>{locale === 'zh' ? '原问题。' : 'Issue. '}</strong><ScientificText text={item.originalIssue} /></p><p><strong>{locale === 'zh' ? '解释。' : 'Explanation. '}</strong><ScientificText text={item.explanation} /></p>{item.correction && <p><strong>{locale === 'zh' ? '修正。' : 'Correction. '}</strong><ScientificText text={item.correction} /></p>}<p><Link href={localizedHref(locale, `/lectures/${String(item.lecture).padStart(2, '0')}/#${item.sectionId}`)}>{locale === 'zh' ? '对应正文' : 'Related lesson'}</Link> · <a href={`${assetPath(`/resources/original/${encodeURIComponent(item.sourceFile)}`)}${item.sourcePage ? `#page=${item.sourcePage}` : ''}`} target="_blank" rel="noreferrer">{locale === 'zh' ? '源文件' : 'Source file'}</a></p></li>)}</ol></main>;
}
