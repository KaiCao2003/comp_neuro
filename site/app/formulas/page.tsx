...
export default function FormulasPage() {
  return <main className="index-page" id="main-content"><header><p className="eyebrow">Formula &amp; Notation Index</p><h1>公式与记号</h1><p className="index-count">{formulas.length}</p></header>
    {course.map((lecture) => { const items = formulas.filter((formula) => formula.lecture === lecture.lecture); return <section className="formula-group" key={lecture.lecture}><h2><Link href={`/lectures/${lecture.slug}/#formulas`}>{lecture.lecture} · {lecture.zhTitle}</Link></h2>{items.map((formula) => <FormulaView formula={formula} linkToLecture key={formula.id} />)}</section>; })}
  </main>;
}
